...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { user, token } = await api<{ user: any; token: string }>("/auth/login", {
        method: "POST",
      ...
      <div className="grid min-h-screen lg:grid-cols-2">
        ...
        <div className="hidden flex-col justify-between bg-brand-black p-12 lg:flex">
          ...
          <div>
            <h2 className="text-4xl font-black text-white" style={{ letterSpacing: "-0.02em" }}>
              Your US company,
              <span className="block text-brand-red">always accessible.</span>
            </h2>
            <p className="mt-4 text-base text-white/60">
              Log in to track your formation progress, manage documents, and view your billing history.
            </p>
            <ul className="mt-8 grid gap-3 text-sm text-white/70">
              {["Formation status tracker", "Secure document vault", "Real-time notifications", "Payment history & receipts"].map((item) => (
        ...
        <div className="flex flex-col items-center justify-center px-6 py-12">
          ...
          <div className="w-full max-w-md">
            <h1 className="text-3xl font-black text-brand-black dark:text-white" style={{ letterSpacing: "-0.02em" }}>Welcome back</h1>
            <p className="mt-2 text-sm text-brand-dark-gray/60 dark:text-white/40">Sign in to access your formation dashboard.</p>
            ...
            <form onSubmit={handleSubmit} className="grid gap-5">
              <div>
                ...
                <input type="email" required placeholder="john@example.com" value={email} onChange={e => setEmail(e.target.value)} className={inputCls} />
              </div>
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <label className="text-sm font-medium text-brand-dark-gray dark:text-white/80">Password</label>
                  <Link href="/forgot-password" className="text-xs text-brand-red hover:underline">Forgot password?</Link>
                </div>
                <div className="relative">
                  <input type={showPw ? "text" : "password"} required placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className={inputCls + " pr-10"} />
            ...
            <p className="mt-6 text-center text-sm text-brand-dark-gray/60 dark:text-white/40">
              ...
              <Link href={`/auth/register${redirectTo !== "/" ? `?redirect=${redirectTo}` : ""}`} className="font-medium text-brand-red hover:underline">
                Create one free
              </Link>
            </p>
          </div>
